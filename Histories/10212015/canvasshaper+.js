...
var CanvasShaper_WorldLimit = function(sceneObject) 
{
	this.SceneObject = sceneObject;
	this.boundMinX = 0;
	this.boundMinY = 0;
	this.boundMaxX = 0;
	this.boundMaxY = 0;
	this.Mode = CANVAS_SHAPER_LIMIT_OFF;
	this.testCallback = 0;
...
var CanvasShaper_Collision = function(Source) 
{
	this.minX = -1;
	this.minY = -1;
	this.maxX = 0;
	...
	this.source = Source;
	this.mode = CANVAS_SHAPER_COLLISION_RECT;
	...
	this.setRadius = function(centerX, centerY, radius) 
	{
		...
		this.radius = radius;
		this.mode = CANVAS_SHAPER_COLLISION_RAD;
		return;
	};
	...
	this.Test = function() 
	{
		...
		if(size >= 1) 
		{
			...
			for(var i=0;i<size;i++) 
			{
				...
				{
					if(this.mode == CANVAS_SHAPER_COLLISION_RECT 
...
var CanvasShaper_Button = function(Canvas, Asset, x, y, width, height) 
{
...
var CanvasShaper_StaticSprite = function(Canvas, Asset, x, y, width, height) 
{
	...
	this.SceneObject.setSize(width, height);
	/** Physics Section */
	this.Physics = new CanvasShaper_Physics(this.SceneObject);
	this.WorldLimit = 0;
	this.Collision = 0;
	...
	{
		if(this.Collision == 0)
			this.Collision = new CanvasShaper_Collision(this.SceneObject);
	...
	this.setWorldLimit = function(minX, minY, maxX, maxY) 
	{
		if(this.WorldLimit == 0)
			this.WorldLimit = new CanvasShaper_WorldLimit(this.SceneObject);
...
var CanvasShaper_AnimeSprite = function(Canvas, Asset, Width, Height) 
{
	Canvas.addChild(this);
	/** get Canvas DOM */
	this.DOMCanvasElement = Canvas.getDOM();
...
var CanvasShaper_Scroller = function(Canvas, Asset) 
{
	Canvas.addChild(this);
	/** get Canvas DOM */
	this.DOMCanvasElement = Canvas.getDOM();
	...
	{
		if(dimension == CANVAS_SHAPER_HORIZONTAL)
			this.direct = {x:1,y:0};
		else 
			this.direct = {x:0,y:1};
		return;
...
var CanvasShaper_Particle = function(Canvas, Asset, Density) 
{
	Canvas.addChild(this);
	/** get Canvas DOM */
	this.DOMCanvasElement = Canvas.getDOM();
	...
	this.loopMode = CANVAS_SHAPER_LOOP_CYCLE;
	...
	{
		this.rotation = CanvasShaper_Math.RadianFromAngle(angle);
		return;
	};
	...
	this.genParticle = function() 
	{
		...
		{
			var lifeTime = CanvasShaper_Math.intRandom(this.LifeMin, this.LifeMax);
			
			/** Calculate the Position of Particles. */
			var positionX = CanvasShaper_Math.intRandom(this.positionXMin, this.positionXMax);
			var positionY = CanvasShaper_Math.intRandom(this.positionYMin, this.positionYMax);
			
			/** Calculate the Alpha of Particles. */
			if(!this.alphaBase)
				this.alphaBase = CanvasShaper_Math.intRandom(1, 100)/100;
			if(this.alphaLife)
			...
			if(!this.scaleXBase)
				this.scaleXBase = CanvasShaper_Math.intRandom(1, 100)/100;
			if(this.scaleXLife) 
			...
			if(!this.scaleYBase)
				this.scaleYBase = CanvasShaper_Math.intRandom(1, 100)/100;
			if(this.scaleYLife) 
			...
			var angularSpeed = CanvasShaper_Math.intRandom(0, this.angularSpeed);
			angularSpeed = CanvasShaper_Math.RadianFromAngle(angularSpeed);
			
			/** Calculator the PositionLife X */
			var deltaPosX = CanvasShaper_Math.intRandom(this.boundMinX, this.boundMaxX);
			deltaPosX = deltaPosX-positionX;
			var deltaPosY = CanvasShaper_Math.intRandom(this.boundMinY, this.boundMaxY);
	...
	this.Draw = function() 
	{
		...
		for(var i=0; i<this.density; i++) 
		{
			...
			if(this.loopMode == CANVAS_SHAPER_LOOP_CYCLE 
			|| this.particle[i].Life < this.particle[i].Lifetime) 
			{
				...
				if(this.emittion) {
					...
					{
						var resetPosX = CanvasShaper_Math.intRandom(this.positionXMin, this.positionXMax);
						var resetPosY = CanvasShaper_Math.intRandom(this.positionYMin, this.positionYMax);
					...
					{
						var moveX = CanvasShaper_Math.intRandom(-5, 5);
						moveX = moveX/10;
						this.particle[i].x += moveX;
						positionX = this.particle[i].x;
						var moveY = CanvasShaper_Math.intRandom(-30, 0);
						moveY = moveY/10;
...
var CanvasShaper_Tile = function(Canvas) 
{
	Canvas.addChild(this);
	/** get Canvas DOM */
	this.DOMCanvasElement = Canvas.getDOM();
	...
	this.WorldLimit = 0;
	/** Tile Section */
	this.align = CANVAS_SHAPER_ALIGN_BOTTOM;
	this.order = CANVAS_SHAPER_ORDER_LEFT;
